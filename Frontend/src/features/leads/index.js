...
function Leads() {
  ...
  const handleFileChange = (e) => {
    ...
    if (selectedFile && allowedFileTypes.includes(selectedFile.type)) {
      ...
      toast.error(
        "Invalid file type. Only .doc, .docx, .pdf, and .txt files are allowed."
      );
    }
  };
  ...
  const generateQuestionPaper = async () => {
    ...
    try {
      ...
      if (
        ...
        response.data.message.includes("Duplicate book detected")
      ) {
        const bookId = response.data.book_id;
        const bookDetailsResponse = await axios.get(
          `${baseUrl}/get-book-by-bookid/${bookId}`,
          {
            withCredentials: true,
          }
        );
  ...
  return (
    <>
      ...
      <TitleCard
        title="Please select a file (accepted types: .doc, .docx, .pdf, .txt)"
        topMargin="mt-6"
      >
        <div
        ...
        {file && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
            <div>
              ...
              {subject && (
                <p className="mt-2 text-sm text-gray-500">
                  Selected Subject: <strong>{subject}</strong>
                </p>
              )}
            ...
            <div>
              <label
                htmlFor="className"
                className="block text-sm mb-2 dark:text-gray-400 font-medium text-gray-700"
              >
                Class Name
              </label>
              <select
              ...
              {className && (
                <p className="mt-2 text-sm text-gray-500">
                  Selected Class: <strong>Class {className}</strong>
                </p>
              )}
              {!className && errorMessage2 && (
                <p className="mt-2 text-sm text-red-500">{errorMessage2}</p>
              )}
            </div>

            <div className="mt-4 flex items-center space-x-4">
              <label className="font-medium dark:text-gray-400 text-gray-700">
                Book Visibility set as{" "}
                <span className="font-bold text-black dark:text-gray-100">
                  {bookIsPublic ? "Public" : "Private"}
                </span>
                <br />
              ...
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`px-3 py-1 rounded-full font-semibold text-white ${
                    responseContent.book.book_ispublic
                      ? "bg-green-500"
                      : "bg-red-500"
                  }`}
                >
      ...
      {showCustomModal && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black  bg-opacity-60">
          <div className="bg-white rounded-xl p-6 w-full max-w-md dark:bg-gray-900 shadow-2xl">
            <h3 className="text-xl font-semibold dark:text-white text-gray-800 mb-4">
              Enter the Subject & Class
            </h3>
            <p className="text-gray-500 dark:text-gray-300 text-sm mb-4">
              Please enter the correct subject and class name. This helps us
              generate accurate questions.
            </p>

            <div className="space-y-4">
              <div>
                <label className="block text-sm mb-2 font-medium text-gray-700 dark:text-gray-400">
                  Subject
                </label>
                <input
                  type="text"
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="E.g., B.Tech, MCA or University..."
                />
              </div>
            </div>
            ...
            <div className="mt-6 flex justify-end space-x-3">
              ...
              <button
                className="btn bg-gradient-to-r from-green-500 to-lime-500 text-white"
                onClick={() => {
                  if (customSubject && customClass) {
                    setSubject(customSubject);
                    setClassName(customClass);
                    setShowCustomModal(false);
                  } else {
                    toast.error("Please fill both custom fields.");
                  }
                }}
              >
                Save
