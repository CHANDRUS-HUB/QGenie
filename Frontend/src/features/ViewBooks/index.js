...
function ViewBook() {
  ...
  const userId = params.id; // Extracting userId from params
    ...
    const fetchBooks = async () => {
      try {
        const response = await axios.get(
          `${baseUrl}/get-books-by-choosen-user/${userId}`,
  ...
  return (
    <div className="min-h-screen  bg-gray-100 dark:bg-gray-900">
      {/* Back Button */}
      <div className="absolute top-20 left-2" onClick={() => window.history.back()}>
        <button
      ...
      <div className="col-span-1 pl-10">
              ...
              <p className="text-gray-500 text-sm dark:text-gray-400">
                {publicbooksList.length} books uploaded by this user.
              </p>
            </div>
        <Toaster />
